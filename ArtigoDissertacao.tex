...
\begin{figure}[ht] \unitlength= 1mm \thicklines
 \centering{
    \begin{picture}(100,6)%115
      \put(0,0){\framebox(98,6)} \put(3,2){$age(X,``youth")$ AND $income(X,``high")  \to classe (X,``A")$}
      \put(0,-7){\framebox(98,6)} \put(3,-5){$age(X,``youth")$ AND $income(X,``low") \to classe (X,``B")$}
      \put(0,-14){\framebox(73,6)} \put(3,-12){$age(X,``middle-aged")$  $\to classe (X,``B")$}
      \put(0,-21){\framebox(62,6)} \put(3,-19){$age(X,``senior")$  $ \to classe (X,``B")$}
      \end{picture}
   }
\end{figure}
